...
def main(_argv):
  ...
  if dplan.get('prefetch_to_device', False):
    ds1 = ds1.apply(tf.data.experimental.prefetch_to_device('/gpu:0', 8))
    ds2 = ds2.apply(tf.data.experimental.prefetch_to_device('/gpu:0', 8))
    ds3 = ds3.apply(tf.data.experimental.prefetch_to_device('/gpu:0', 8))
